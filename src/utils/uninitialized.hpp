...
template <typename T> class Uninitialized {
  public:
    ...
    template <typename U>
    requires(std::is_constructible_v<T, U>) Uninitialized(U &&value)
    noexcept(std::is_nothrow_constructible_v<T, U>) {
        std::construct_at(&this->value, std::forward<U>(value));
    }

    template <typename U>
    requires(std::is_constructible_v<T, U>) Uninitialized &
    ...
    requires(std::constructible_from<T, Args...>) auto emplace(Args &&...args) noexcept(
