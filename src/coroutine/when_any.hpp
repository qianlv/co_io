...
template <typename T>
inline Task<void> when_any_helper(const auto &task, WhenAnyResult<T> &result, size_t index) {
    using return_type = typename std::decay_t<decltype(task)>::return_type;
    if constexpr (std::is_void_v<return_type>) {
        co_await task;
...
requires(HasReturnType<Awaiters> &&...) inline Task<WhenAnyResult<std::variant<typename VoidType<
    typename Awaiters::return_type>::type...>>> when_any_impl(std::index_sequence<Is...>,
                                                              Awaiters &&...tasks) {
    auto self = co_await Self<>{};
    WhenAnyResult<std::variant<typename VoidType<typename Awaiters::return_type>::type...>>
        results{};
...
requires(HasReturnType<Awaiter>) inline Task<WhenAnyResult<typename VoidType<
    typename Awaiter::return_type>::type>> when_any(const std::vector<Awaiter> &tasks) {
    auto self = co_await Self<>{};
    WhenAnyResult<typename VoidType<typename Awaiter::return_type>::type> results{};
...
requires(sizeof...(Awaiters) > 0) &&
    (HasReturnType<Awaiters> && ...) inline Task<WhenAnyResult<std::variant<typename VoidType<
