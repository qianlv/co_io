...
template <typename... Ts> struct WhenAllResult { std::tuple<Ts...> value{}; };
...
inline Task<void> when_all_helper(const auto &task, WhenAllResult<Ts...> &result,
                                  size_t &complete_count, std::coroutine_handle<> previous_handle) {
...
requires(HasReturnType<Awaiters> &&...) inline Task<WhenAllResult<typename VoidType<
    typename Awaiters::return_type>::type...>> when_all_impl(std::index_sequence<Is...>,
                                                             Awaiters &&...tasks) {
    auto self = co_await Self<>{};
    WhenAllResult<typename VoidType<typename Awaiters::return_type>::type...> results{};
    size_t complete_count = 0;
    Task<void> new_tasks[]{when_all_helper<Is>(tasks, results, complete_count, self)...};
...
requires(sizeof...(Awaiters) > 0) &&
    (HasReturnType<Awaiters> && ...) inline Task<WhenAllResult<
        typename VoidType<typename Awaiters::return_type>::type...>> when_all(Awaiters &&...tasks) {
